...
int Player::currentPos()
{
  ...
  if (forward_h  && left_F)  return 2;
  if (forward_F  && left_F)  return 3;
  if (forward_F  && left_h)  return 4;
  if (forward_F  && yNull)   return 5;
  if (forward_F  && right_h) return 6;
  if (forward_F  && right_F) return 7;
  if (forward_h  && right_F) return 8;
  if (xNull      && right_F) return 9;
  if (backward_h && right_F) return 10;
  if (backward_F && right_F) return 11;
  if (backward_F && right_h) return 12;
  if (backward_F && yNull)   return 13;
  if (backward_F && left_h)  return 14;
  if (backward_F && left_F)  return 15;
  if (backward_h && left_F)  return 16;
  return 0;
